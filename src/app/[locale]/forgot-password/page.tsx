    ...
    const handleSubmit = useCallback(
        async (e: React.FormEvent) => {
            ...
            try {
                ...
                if (res?.error !== null) {
                    const firstMessage =
                        Array.isArray(res.error?.messages) && res.error!.messages!.length > 0
                            ...
                            : mapErrorToUserMessage(res.error as ErrorInput) || t('errorSendingEmail');
                    if (!mountedRef.current) return;
                ...
                const success = res?.data?.message || t('checkEmailSuccess');
                if (!mountedRef.current) return;
                setSuccessMessage(success);
            } catch (err: unknown) {
                if (!mountedRef.current) return;
                // Use error mapper for consistent error handling
                const errorMessage = mapErrorToUserMessage(err as ErrorInput) || t('genericError');
        ...
        <div
            ...
            <div
                ...
                <header className="text-center mb-10">
                    ...
                    <h1 className="text-4xl font-bold text-blue-800 mb-2">{t('forgotPasswordTitle')}</h1>
                    <p className="text-gray-600 text-lg">{t('forgotPasswordSubtitle')}</p>
                </header>

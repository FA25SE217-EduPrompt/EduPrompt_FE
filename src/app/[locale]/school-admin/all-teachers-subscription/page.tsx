...
import { TeacherSubscriptionUsageDto } from '@/types/school-admin.api';
import { mapErrorToUserMessage } from '@/utils/errorMapper';

export default function AllTeachersSubscriptionPage() {
    const [teachers, setTeachers] = useState<TeacherSubscriptionUsageDto[]>([]);
    ...
    return (
        <div className="p-6">
            <div className="mb-6">
                <h1 className="text-2xl font-bold text-gray-900">All Teachers Subscription Usage</h1>
                <p className="text-gray-600 mt-1">
                    View subscription usage breakdown for all teachers
                </p>
            </div>
                                ...
                                <tbody className="bg-white divide-y divide-gray-200">
                                    {teachers && teachers.length > 0 ? (
                                        teachers.map((teacher) => (
                                            ...
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-900">
                                                    {teacher.testsUsed.toLocaleString()}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="text-sm text-gray-900">
                                                    {teacher.optimizationsUsed.toLocaleString()}
                                                </div>
                                            </td>
                                            <td className="px-6 py-4 whitespace-nowrap">
                                                <div className="flex items-center">
                                                    <div className="w-full">
                                                        <div className="flex items-center justify-between mb-1">
                                                            <span className={`text-sm ${getPercentageColor(teacher.percentage)}`}>
                                                                {teacher.percentage.toFixed(1)}%
                                                            </span>
                                                        </div>
                                                        <div className="w-full bg-gray-200 rounded-full h-2">
                                                            <div
                                                                className={`h-2 rounded-full ${getProgressBarColor(teacher.percentage)}`}
                                        ...
                                        <tr>
                                            <td colSpan={6} className="px-6 py-8 text-center text-gray-500">
                                                No teachers subscription data available
                                            </td>
                                        </tr>
                                    )}
                                </tbody>

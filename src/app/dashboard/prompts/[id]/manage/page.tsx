...
export default function PromptManagePage() {
    ...
    const updateMetadataMutation = useMutation({
        ...
        onSuccess: () => {
            toast.success("Prompt metadata updated");
    ...
    const shareMutation = useMutation({
        mutationFn: () => promptsService.sharePrompt(promptId),
        onSuccess: (response) => {
    ...
    const rollbackMutation = useMutation({
        ...
        onSuccess: () => {
            toast.success("Prompt rolled back to version");
            queryClient.invalidateQueries({ queryKey: ['prompts', 'detail', promptId] });
            queryClient.invalidateQueries({ queryKey: ['promptVersions', promptId] });
        },
        onError: () => toast.error("Failed to rollback prompt"),
    });
    ...
    const handleOptimize = (model: PromptAiModel, input: string) => {
        optimizeMutation.mutate({
            ...
            onSuccess: () => {
                toast.success("Optimization started. Check your notifications for results.");
            },
    ...
    const handleRollback = (versionId: string) => {
        if (confirm("Are you sure you want to rollback to this version? Current changes will be saved as a new version.")) {
    ...
    if (!isAuthenticated || !prompt) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center text-gray-500">
                <p>Prompt not found or access denied.</p>
